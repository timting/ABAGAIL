...
class Analyze implements Runnable {
    ...
    private double calculate_accuracy(Instance[] instances, Instance optimalInstance) {
        ...
        System.out.println("Instance length is " + instances.length);
        ...
        try {
            switch (threadName) {
                case "RHC":
                    ...
                    break;
                case "SA":
                    oa = new SimulatedAnnealing(300, .95, nnop);
                    break;
                case "GA":
                    oa = new StandardGeneticAlgorithm(200, 100, 100, nnop);
                    break;
            }
...
public class NNTrain{
    public static void main(String[] args) {
        ...
        String training_data_files[] = {
                "adult.bucket.binary.train.csv"
        };
        String test_data_files[] = {
                "adult.bucket.binary.test.csv"
        };

        int num_runs = 3; //This starts a new run at a new random point or random population
        int training_iterations[] = {750};
